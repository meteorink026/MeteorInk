  ...
  function installMobile(){
    ...
    if(!menu){
      ...
      menu.innerHTML=nav.map(([label,href])=>`<a class="${activeFor(label)?'active':''}" href="${href}">${label}</a>`).join('')+
        (currentUser
          ? '<div class="mobile-account"><div class="mobile-user">'+esc(currentUser.name||currentUser.email||'User')+'</div><a href="profile.html">My Profile</a><a href="index.html#library">My Library</a><a href="author-dashboard.html">Author Dashboard</a><button type="button" class="mobile-logout" id="mobileLogoutBtn">Log Out</button></div>'
